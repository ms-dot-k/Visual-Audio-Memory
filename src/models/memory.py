...
class Memory(nn.Module):
    ...
    def forward(self, query, value=None, inference=False):
        ...
        key_norm = F.normalize(self.key, dim=1) #88,512
        embd_query = self.q_embd(mer_query)
        key_sim = F.linear(F.normalize(embd_query, dim=1), key_norm)  # BS, 88
        key_add = self.softmax(self.radius * key_sim)  # BS, 88

        vir_aud = torch.matmul(key_add, self.value.detach()) # BS, 512
        ...
        if not inference:
            ...
            value_norm = F.normalize(self.value, dim=1) #88,512
            value_sim = F.linear(F.normalize(embd_value, dim=1), value_norm) #BS, 88
